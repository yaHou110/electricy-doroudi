...
Practical runbook for running Droudian Platform in development and on a single VPS. Everything here reflects the actual scripts in `package.json` and the compose file at the repo root.
...
| --- | --- | --- |
| `DATABASE_URL` | Prisma connection string | `postgresql://postgres:postgres@localhost:5432/droudian?schema=public` in dev; use the VPS-internal address in production. |
...
docker compose up -d          # PostgreSQL 16 + healthcheck, volume droudian-postgres
npm install
...
* **Backups**: `docker exec <postgres container> pg_dump -U postgres -Fc droudian > backup-$(date +%F).dump`. Restore with `pg_restore -U postgres -d droudian --clean backup.dump`. Schedule daily dumps off-box; none are automated yet.
...
* **Watch for**: `503 SERIALIZATION_RETRY_EXHAUSTED` responses (sustained concurrency pressure), slow `/api/dashboard` (full-table derivation), and disk growth of the `droudian-postgres` volume.
